...
const Tab = ({activeTab,setActiveTab}) => {
    return (
        ...
        <div className={`max-width ${classes.options}`} >
            {tabs.map((tab) => {
                return <div onClick={() => setActiveTab(tab.name)} className={`${classes.tabItem} absolute-center cur-po 
                 ${activeTab === tab.name && classes.activeTab}`}>
                    <div className={`${classes.tabImageContainer} absolute-center`}
                    style={{backgroundColor:`${activeTab === tab.name ? tab.backdrop : ""}`}}
                    >
                    <img 
                    alt={tab.name}
                    className={classes.tabImage} src={activeTab === tab.name ? tab.active_img:tab.inactive_img} />
                </div>
